...
namespace OrpheusCore
{
    ...
    public class OrpheusTable<T> : IOrpheusTable<T>
    {
        ...
        private readonly IOrpheusDatabase database;
        ...
        /// <summary>
        /// Resolved once at construction rather than per row: key generation sits in the per-record
        /// insert path.
        /// </summary>
        ...
        /// <summary>
        /// Guards operations that need a WHERE clause built from the model's key fields.
        /// </summary>
        /// <remarks>
        /// Loading does not need keys — a model used purely as a projection target for
        /// <c>SQL&lt;T&gt;</c>/<c>SQLAsync&lt;T&gt;</c> is perfectly valid without a
        /// <c>[PrimaryKey]</c>. Updating or deleting one is not, because there would be no way to
        /// identify the row. This is checked here, at the point of use, rather than when the table
        /// is constructed.
        /// </remarks>
        /// <param name="operation">The operation being attempted, for the error message.</param>
        private void requireKeyFields(string operation)
        {
            if (this.KeyFields.Count == 0)
            {
                throw new InvalidOperationException(
                    $"Cannot {operation} table {this.Name}: model {typeof(T).Name} declares no key fields. " +
                    "Mark a property with [PrimaryKey], or pass key fields when creating the table. " +
                    "(Models used only to receive query results do not need keys.)");
            }
        }
        ...
        {
            this.requireKeyFields("delete from");
                                ...
                                case ModifiedRecordAction.mraInsert:
                                    {
                                        ...
                                        {
                                            paramValue = this.keyGenerator.NewKey();
            ...
            )
        {
            ...
            // A missing logger must never be why a table fails to construct — this method logs
            // warnings and errors below before it is done.
            this.logger = logger ?? database.LoggerFactory.CreateLogger<IOrpheusTable<T>>();
            this.Name = tableName;
            ...
            this.modelHelper = OrpheusModelHelper.GetOrAdd(typeof(T));
            ...
            this.keyGenerator = this.database.KeyGenerator;
            // Only the load command is prepared up front. Insert/update/delete SQL is built per
            // batch by buildBatchedInsertCommand and friends, so nothing here needs key fields —
            // which is what lets a keyless read-only projection be loaded via SQL<T>/SQLAsync<T>.
            // Mutating a keyless table is rejected at the point of use instead; see requireKeyFields.
