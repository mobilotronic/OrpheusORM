...
namespace OrpheusTestModels
{
    ...
    /// <summary>
    /// Model with a DB-generated (identity) primary key. Orpheus's schema builder does not
    /// create identity columns, so the underlying table must be created manually — see
    /// SQLServerBatchedInsertKeyRetrievalTests for how this model's table is provisioned.
    /// </summary>
    public class TestModelIdentityKeyed
    {
        [PrimaryKey(isAutoGenerated: false, isDBGenerated: true)]
    ...
    /// <summary>
    /// Composite-key model with no FK dependencies, used to exercise the multi-key-field
    /// WHERE-clause building in batched update/delete (everywhere else in the test suite
    /// only single-key models are used).
    /// </summary>
    [PrimaryCompositeKey(new string[] { "KeyPart1", "KeyPart2" })]
    public class TestModelCompositeKeyed
    {
        public Guid KeyPart1 { get; set; }
        public Guid KeyPart2 { get; set; }
