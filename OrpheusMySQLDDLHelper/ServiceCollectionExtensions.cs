...
{
    /// <summary>
    /// Registers the services needed for an <see cref="IOrpheusDatabase"/> backed by MySQL,
    /// mirroring the EF Core "UseMySql" provider-registration pattern.
    /// </summary>
    public static class ServiceCollectionExtensions
    {
        /// <summary>
        /// Registers a pooled MySQL <see cref="IOrpheusConnectionFactory"/>, the MySQL
        /// <see cref="IOrpheusDDLHelper"/>, and <see cref="IOrpheusDatabase"/> itself, using the
        /// supplied connection configuration.
        /// </summary>
        public static IServiceCollection AddOrpheusMySql(this IServiceCollection services, IDatabaseConnectionConfiguration connectionConfiguration)
        {
            if (connectionConfiguration == null)
                throw new ArgumentNullException(nameof(connectionConfiguration));

            services.AddOrpheusServices();
            services.AddTransient<IOrpheusConnectionFactory>(_ => new MySqlConnectionFactory { ConnectionConfiguration = connectionConfiguration });
            services.AddTransient<IOrpheusDDLHelper, OrpheusMySQLServerDDLHelper>();
        ...
        /// <summary>
        /// Registers Orpheus for MySQL, building the connection configuration inline.
        /// </summary>
        public static IServiceCollection AddOrpheusMySql(this IServiceCollection services, Action<IDatabaseConnectionConfiguration> configureConnection)
        {
            ...
            return services.AddOrpheusMySql(config);
        }

        /// <summary>
        /// Registers Orpheus for MySQL, resolving the named connection out of an
